    ...
    const experiences: Experience[] = [
        {
            ...
            responsibilities: "Extend and improve MediaRadar's binary gathering applications, supporting services, and database procedures. Oversee upgrade of internal tools portal frontend, from angular 1.5 to angular 15. Architect series of services to consume Large Datasets from 3rd parties (up to 10mil rows/day). Package and deploy release builds to remote servers. "
        },
        {
...
<div class="resume">
    ...
    <div class="experience">
        <div><label class="company">{exp.company}</label></div>
        <div><label class="specialty">{exp.specialty}</label></div>
        <div><label class="title">{exp.title}</label></div>
        <div><p class="responsibilities">{exp.responsibilities}</p></div>
    </div>
    {/each}
</div>
